...
for pattern in "$@"; do
  for file in $pattern; do
    [[ -f "$file" ]] || continue
    [[ -L "$file" ]] && { log "Skipping symlink: $file"; continue; }
    ...
    [[ -s "$temp_file" ]] || { rm -f "$temp_file"; die "Transformation produced empty output: $relative"; }

    # Detect actual changes
    if ! cmp -s "$file" "$temp_file"; then
      mv "$temp_file" "$file" || { rm -f "$temp_file"; die "Atomic replace failed: $relative"; }
