...
rollback_restore() {
  ...
  cp -a "$BACKUP_DIR"/* /etc/apt/ || { log ERROR "Rollback copy failed"; return 1; }
...
detect_and_build_url() {
  local release; release=$(lsb_release -cs 2>/dev/null || echo "unknown")
...
purge_and_configure() {
  ...
  if [[ "$FORCE" != true ]] && [[ -f "$MARKER_FILE" ]]; then
    local pd; pd=$(cat "$MARKER_FILE" 2>/dev/null || echo "unknown")
    ...
    for repo in "${ENTERPRISE_REPOS[@]}"; do
      [[ -f "$repo" ]] && { log WARN "Enterprise repo reappeared: $repo (re-purging)"; break; }
    done
    if [[ -f "$COMMUNITY_REPO" ]]; then
      local current; current=$(cat "$COMMUNITY_REPO" 2>/dev/null || echo "")
      if [[ "$current" == "$url" ]]; then
  ...
  for repo in "${ENTERPRISE_REPOS[@]}"; do
    if [[ -f "$repo" ]]; then
      local content; content=$(cat "$repo" 2>/dev/null || echo "<empty>")
  ...
  local test_url; test_url=${url#deb }; test_url=${test_url%% *}
...
refresh_and_verify() {
  ...
  local apt_output; apt_output=$(mktemp)
  ...
  if ! apt update >"$apt_output" 2>&1; then
    ...
    log ERROR "apt update output:"; sed 's/^/  /' <"$apt_output"
    return $rc
  fi

  # Verification
  for repo in "${ENTERPRISE_REPOS[@]}"; do
    [[ -f "$repo" ]] && { log ERROR "Enterprise repo still exists: $repo"; return 1; }
  done
  if grep -r "enterprise.proxmox.com" /etc/apt/sources.list* 2>/dev/null; then
    log ERROR "Enterprise URLs found in sources"; return 1
  fi
  [[ -f "$COMMUNITY_REPO" ]] || { log ERROR "Community repo missing: $COMMUNITY_REPO"; return 1; }
  if ! grep -q "^deb.*pve-no-subscription" "$COMMUNITY_REPO"; then
    log ERROR "Community repo not properly configured"; return 1
  fi
  if ! apt-cache policy >/dev/null 2>&1; then
    log ERROR "apt-cache policy failed (repo corruption?)"; return 1
  fi
